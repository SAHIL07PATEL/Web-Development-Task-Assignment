/* eslint-disable no-unused-vars */
/* eslint-disable react/jsx-key */
import React, { useState } from "react";


let dummydata = [
  { id: 1, name: "Egge"},
  { id: 2, name: "Milk"},
  { id: 3, name: "Chesse"},
]

const Grocery = () => {

  const [item,setItem] = useState(dummydata);


  return (
    <>
      <div className="flex items-center justify-center min-h-screen bg-gradient-to-r from-blue-400 to-purple-400">
        <div className="bg-gray-800 rounded-lg p-8 shadow-lg w-full max-w-md">
          <h1 className="text-white text-2xl font-bold mb-6 text-center">
            Grocery Shopping
          </h1>
          <div className="space-y-4">
            <div className="flex space-x-4">
              <input
                type="text"
                placeholder="Add something to your list"
                className="w-full px-4 py-2 rounded bg-gray-600 text-gray-400 focus:outline-none"
              />
              <button className="px-6 py-2 rounded bg-gradient-to-r from-purple-500 to-blue-500 text-white">
                Add
              </button>
            </div>

            {
              item.map((item) => (
                <div key={item.id}>
                  <div className="flex items-center space-x-4">
                  <label  className="w-full text-center px-4 py-2 rounded bg-gradient-to-r from-purple-500 to-blue-500 text-white focus:outline-none">{item.name}</label>
                  <button className="text-white hover:text-red-500">
                    <i className="ri-delete-bin-6-line" />
                  </button>
                  <button className="text-white hover:text-yellow-500">
                    <i className="ri-edit-box-line" />
                  </button>
                </div>
                </div>
              ))
            }

            <div className="flex justify-center">
              <button className="mt-4 px-6 py-2 rounded bg-white text-gray-800">
                Delete List
              </button>
            </div>
          </div>
        </div>
      </div>
    </>
  );
};

export default Grocery;
Grocery;


























// import React, { useState } from "react";

// let dummydata = [
//   { id: 1, name: "Eggs" },
//   { id: 2, name: "Milk" },
//   { id: 3, name: "Cheese" },
// ];

// const Grocery = () => {
//   const [items, setItems] = useState(dummydata);
//   const [editingItemId, setEditingItemId] = useState(null); // Track which item is being edited
//   const [newName, setNewName] = useState(""); // Track input value

//   const handleEdit = (item) => {
//     setEditingItemId(item.id); // Set the item being edited
//     setNewName(item.name); // Set the input value to the current name
//   };

//   const handleInputChange = (e) => {
//     setNewName(e.target.value); // Update the input value
//   };

//   const saveEdit = (id) => {
//     setItems(
//       items.map((item) =>
//         item.id === id ? { ...item, name: newName } : item
//       )
//     );
//     setEditingItemId(null); // Stop editing mode
//   };

//   return (
//     <div className="flex items-center justify-center min-h-screen bg-gradient-to-r from-blue-400 to-purple-400">
//       <div className="bg-gray-800 rounded-lg p-8 shadow-lg w-full max-w-md">
//         <h1 className="text-white text-2xl font-bold mb-6 text-center">
//           Grocery Shopping
//         </h1>
//         <div className="space-y-4">
//           <div className="flex space-x-4">
//             <input
//               type="text"
//               placeholder="Add something to your list"
//               className="w-full px-4 py-2 rounded bg-gray-600 text-gray-400 focus:outline-none"
//             />
//             <button className="px-6 py-2 rounded bg-gradient-to-r from-purple-500 to-blue-500 text-white">
//               Add
//             </button>
//           </div>

//           {items.map((item) => (
//             <div key={item.id}>
//               <div className="flex items-center space-x-4">
//                 {editingItemId === item.id ? (
//                   // Render input when in edit mode
//                   <input
//                     type="text"
//                     value={newName}
//                     onChange={handleInputChange}
//                     className="w-full px-4 py-2 rounded bg-gray-600 text-white focus:outline-none"
//                   />
//                 ) : (
//                   // Render label when not in edit mode
//                   <label className="w-full text-center px-4 py-2 rounded bg-gradient-to-r from-purple-500 to-blue-500 text-white focus:outline-none">
//                     {item.name}
//                   </label>
//                 )}
//                 {editingItemId === item.id ? (
//                   // Show save button when in edit mode
//                   <button
//                     onClick={() => saveEdit(item.id)}
//                     className="text-white hover:text-green-500"
//                   >
//                     Save
//                   </button>
//                 ) : (
//                   <>
//                     <button className="text-white hover:text-red-500">
//                       <i className="ri-delete-bin-6-line" />
//                     </button>
//                     <button
//                       onClick={() => handleEdit(item)}
//                       className="text-white hover:text-yellow-500"
//                     >
//                       <i className="ri-edit-box-line" />
//                     </button>
//                   </>
//                 )}
//               </div>
//             </div>
//           ))}

//           <div className="flex justify-center">
//             <button className="mt-4 px-6 py-2 rounded bg-white text-gray-800">
//               Delete List
//             </button>
//           </div>
//         </div>
//       </div>
//     </div>
//   );
// };

// export default Grocery;
